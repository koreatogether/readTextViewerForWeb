...
let pageAnchorLine  = 0;
let logicalLinesPerSpread = 0;
...
function updateLayout({ preserveLogicalPages = false } = {}) {
    applyLayoutMode();
    linesPerPage = calcLinesPerPage();
    if (!preserveLogicalPages || !logicalLinesPerSpread) {
        logicalLinesPerSpread = linesPerPage * (isSinglePageMode() ? 1 : 2);
        const maxPageIndex = Math.max(0, Math.ceil(lines.length / logicalLinesPerSpread) - 1);
        pageIndex = Math.max(0, Math.min(maxPageIndex, pageIndex));
        pageAnchorLine = pageIndex * logicalLinesPerSpread;
    }
    recalcSearch();
    renderCurrentPage();
}
...
function getTotalPages() {
    const linesPerSpread = logicalLinesPerSpread || linesPerPage * (isSinglePageMode() ? 1 : 2);
    return Math.max(1, Math.ceil(lines.length / linesPerSpread));
}

function getCurrentLinesPerSpread() {
    return logicalLinesPerSpread || linesPerPage * (isSinglePageMode() ? 1 : 2);
}
...
function updatePageInfo() {
    ...
    if (pageModeLabel) pageModeLabel.textContent = mode;
    if (pageNumberInput) pageNumberInput.max = String(getTotalPages());
    if (pageNumberInput && document.activeElement !== pageNumberInput) {
        pageNumberInput.value = String(pageIndex + 1);
    }
    if (pageTotalPages) pageTotalPages.textContent = String(getTotalPages());
}

function jumpToPageFromInput() {
    if (!pageNumberInput) return;
    const target = Number.parseInt(pageNumberInput.value, 10);
...
function gotoPage(n) {
    ...
    const linesPerSpread = getCurrentLinesPerSpread();
    pageAnchorLine = pageIndex * linesPerSpread;
    renderCurrentPage();
}
...
function addRecentFile(fileName, text, pi) {
    if (!fileName) return;
    const position = typeof pi === "object" && pi
        ? pi
        : { pageIndex: Number.isFinite(pi) ? pi : 0, anchorLine: Number.isFinite(pi) ? pi * getCurrentLinesPerSpread() : 0 };
    try {
        ...
        arr.unshift({
            ...
            anchorLine: Number.isFinite(position.anchorLine) ? position.anchorLine : 0,
...
function saveState() {
    ...
    try {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({
            currentFileName, pageIndex, pageAnchorLine,
            logicalLinesPerSpread,
    ...
    if (currentFileName) {
        try {
            ...
            if (item) {
                ...
                item.anchorLine = pageAnchorLine;
                localStorage.setItem(RECENT_FILES_KEY, JSON.stringify(arr));
                renderRecentFilesUI();
            }
        } catch { /* ignore */ }
...
function setLinesFromText(text, fileName, pi) {
    ...
    logicalLinesPerSpread = linesPerPage * (isSinglePageMode() ? 1 : 2);
    const maxPageStart = Math.max(0, lines.length - logicalLinesPerSpread);
    const savedAnchor = pi && typeof pi === "object"
        ? (Number.isFinite(pi.anchorLine) ? pi.anchorLine : Number.isFinite(pi.pageIndex) ? pi.pageIndex * logicalLinesPerSpread : 0)
        : Number.isFinite(pi) ? pi * logicalLinesPerSpread : 0;
    pageAnchorLine = Math.max(0, Math.min(maxPageStart, savedAnchor));
    pageIndex      = Math.max(0, Math.min(getTotalPages() - 1, Math.floor(pageAnchorLine / logicalLinesPerSpread)));
    recalcSearch();
    renderCurrentPage();
    if (fileName) addRecentFile(fileName, text, { pageIndex, anchorLine: pageAnchorLine });
}
...
function getSavedPageForFile(fileName) {
    try {
        // 1. 메인 상태 (마지막으로 읽던 파일)
        const meta = JSON.parse(localStorage.getItem(STORAGE_KEY) || "null");
        if (meta && meta.currentFileName === fileName) {
            return {
                pageIndex: meta.pageIndex || 0,
                anchorLine: Number.isFinite(meta.pageAnchorLine) ? meta.pageAnchorLine : (meta.pageIndex || 0) * (meta.logicalLinesPerSpread || 1)
            };
        }
    } catch { /* ignore */ }
    try {
        ...
        if (item) {
            return {
                ...
                anchorLine: Number.isFinite(item.anchorLine) ? item.anchorLine : (item.pageIndex || 0) * (logicalLinesPerSpread || 1)
            };
        }
    } catch { /* ignore */ }
    return { pageIndex: 0, anchorLine: 0 };
}
...
function loadLastSession() {
    ...
    setLinesFromText(textToLoad, currentFileName, state);
    renderBookmarkList();
}
...
function init() {
    ...
    if (recentFilesList) {
        recentFilesList.addEventListener("change", () => {
            ...
            if (item) {
                if (item.text) {
                    setLinesFromText(item.text, item.fileName, item);
                } else {
    ...
    if (jumpPageBtn && pageNumberInput) {
        jumpPageBtn.addEventListener("click", jumpToPageFromInput);
    ...
    window.addEventListener("resize", () => {
        ...
        resizeTimer = setTimeout(() => updateLayout({ preserveLogicalPages: true }), 80);
    });

    // 종료/백그라운드 전환 직전에도 마지막 상태 저장
    const flushState = () => saveState();
